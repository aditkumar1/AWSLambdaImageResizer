...
import com.awslearning.model.ResizerInput;
...
public class LambdaHandlerMain implements RequestHandler<ResizerInput,String> {

    AmazonS3 s3client;
    @Override
    public String handleRequest(ResizerInput input, Context context) {
        String resizedurl = createUrl(input, context);
        if (!alreadyExists(resizedurl)) {
            BufferedImage originalImage = readImage(input, context);
            if (originalImage != null) {
                InputStream resizedImage = resizeImage(originalImage, input, context);
